...
__private_extern__ float rsLength(float *xyz){
	return(float(sqrt(xyz[0] * xyz[0] + xyz[1] * xyz[1] + xyz[2] * xyz[2])));
}


__private_extern__ float rsNormalize(float *xyz){
...
__private_extern__ float rsDot(float *xyz1, float *xyz2){
	return(xyz1[0] * xyz2[0] + xyz1[1] * xyz2[1] + xyz1[2] * xyz2[2]);
}


__private_extern__ void rsCross(float *xyz1, float *xyz2, float *xyzOut){
...
__private_extern__ void rsScaleVec(float *xyz, float scale){
	xyz[0] *= scale;
	xyz[1] *= scale;
	xyz[2] *= scale;
}