...
int myRandi(int x){return(rand() * x / 32767);}
float myRandf(float x){return(float(rand()) * x / 32767.0f);}
...
void IdleProc(){
	...
	// Limit to 30 fps
	if(elapsedTime > 0.0333f){
		while(elapsedTime > 0.0333f)
			elapsedTime -= 0.0333f;
...
void doSaver(HWND hwnd){
	...
	for(i=0; i<NUMCONSTS; i++){
		ct[i] = myRandf(PIx2);
		cv[i] = myRandf(0.005f * float(dSpeed)) + 0.0001f;
	}
