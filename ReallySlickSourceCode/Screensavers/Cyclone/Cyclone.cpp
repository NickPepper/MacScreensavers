...
void cyclone::update(){
	...
	if(xyzChange[temp][0] == xyzChange[temp][1]){
		...
		xyzChange[temp][0] = 0;
		xyzChange[temp][1] = myRandi(10000 / dSpeed) + 5000 / dSpeed;
	}
	temp = dComplexity + 1;
	if(xyzChange[temp][0] == xyzChange[temp][1]){
		oldxyz[temp][0] = xyz[temp][0];
		...
		xyzChange[temp][0] = 0;
		xyzChange[temp][1] = myRandi(7000 / dSpeed) + 5000 / dSpeed;
	}
	for(i=dComplexity; i>1; i--){
		if(xyzChange[i][0] == xyzChange[i][1]){
			...
			if(targetxyz[i][1] < -high)
				targetxyz[i][1] = -high;
			xyzChange[i][0] = 0;
			xyzChange[i][1] = myRandi(5000 / dSpeed) + 3000 / dSpeed;
		}
	}
	if(xyzChange[1][0] == xyzChange[1][1]){
		...
		xyzChange[1][0] = 0;
		xyzChange[1][1] = myRandi(4000 / dSpeed) + 2000 / dSpeed;
	}
	if(xyzChange[0][0] == xyzChange[0][1]){
		...
		xyzChange[0][0] = 0;
		xyzChange[0][1] = myRandi(3000 / dSpeed) + 1500 / dSpeed;
	}
	for(i=0; i<(dComplexity+3); i++){
	...
	if(widthChange[temp][0] == widthChange[temp][1]){
		...
		widthChange[temp][1] = myRandi(5000 / dSpeed) + 5000 / dSpeed;
	}
	temp = dComplexity + 1;
	if(widthChange[temp][0] == widthChange[temp][1]){
		oldWidth[temp] = width[temp];
	...
	for(i=dComplexity; i>1; i--){
		if(widthChange[i][0] == widthChange[i][1]){
			...
			widthChange[i][0] = 0;
			widthChange[i][1] = myRandi(5000 / dSpeed) + 4000 / dSpeed;
		}
	}
	if(widthChange[1][0] == widthChange[1][1]){
		...
		widthChange[1][0] = 0;
		widthChange[1][1] = myRandi(5000 / dSpeed) + 3000 / dSpeed;
	}
	if(widthChange[0][0] == widthChange[0][1]){
		oldWidth[0] = width[0];
		targetWidth[0] = myRandf(15.0f) + 5.0f;
		widthChange[0][0] = 0;
		widthChange[0][1] = myRandi(5000 / dSpeed) + 2000 / dSpeed;
	}
	for(i=0; i<(dComplexity+3); i++){
...
void particle::update(){
	...
	newStep = (0.005f * float(dSpeed)) / (width * width * cyWidth);
	step += newStep;
...
void initSaver(HWND hwnd){
	...
	if(!myRandi(500)){  // Easter egg view
		glRotatef(90, 1, 0, 0);
